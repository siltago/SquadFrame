...
export async function GET(request: NextRequest) {
  ...
  const { data: user } = await admin.from("usuarios").select("id,nome,email,cargo:cargos(id,is_admin)").eq("auth_id", auth.user.id).eq("ativo", true).maybeSingle();
  ...
  return NextResponse.json({
    user: { id: user.id, name: user.nome, email: user.email }, permissions,
