    ...
    <>
      {podeExcluir && (
        <div className="flex justify-end mb-2">
          {!modoExcluir ? (
            ...
            <button onClick={cancelar} className="text-xs text-ink-faint hover:text-ink underline">Cancelar exclusão</button>
          )}
        </div>
      )}
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {pedidos.length === 0 ? (
              <tr><td colSpan={modoExcluir ? 6 : 5} className="px-5 py-10 text-center text-sm text-ink-faint">Nenhum pedido encontrado.</td></tr>
            ) : (
              pedidos.map((p) => (
                <tr key={p.id} className={`border-b border-line last:border-0 ${selecionados.has(p.id) && modoExcluir ? "bg-red-50 dark:bg-red-900/10" : "hover:bg-canvas"}`}>
                  {modoExcluir && (
                  ...
                  <td className="px-5 py-3">
                    <Link href={`/compras/pedidos/${p.id}`} className="font-mono text-xs font-semibold text-steel hover:underline">{p.numero}</Link>
                  </td>
                  <td className="px-5 py-3 text-ink-soft">{p.fornecedor?.nome ?? "—"}</td>
                  <td className="px-5 py-3 text-ink-soft">{p.obra?.nome ?? "—"}</td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3 text-xs text-ink-faint">{new Date(p.criado_em).toLocaleDateString("pt-BR")}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      {modoExcluir && n > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-between border-t border-red-200 bg-red-50 px-8 py-3 shadow-lg dark:bg-red-900/20">
          <p className="text-sm font-medium text-red-700 dark:text-red-400">{n} pedido(s) selecionado(s)</p>
