...
function identificarGargalos(
  ...
  for (const p of pedidos) {
    ...
    if (dias >= 3) g.push({ nivel: "critico", descricao: `Pedido ${p.numero} aguardando aprovação`, detalhe: `${dias} dia${dias !== 1 ? "s" : ""} parado`, href: `/compras/pedidos/${p.id}`, linkLabel: "Ver pedido" });
    else if (dias >= 1) g.push({ nivel: "atencao", descricao: `Pedido ${p.numero} aguardando aprovação`, detalhe: `${dias} dia${dias !== 1 ? "s" : ""} aguardando`, href: `/compras/pedidos/${p.id}`, linkLabel: "Ver pedido" });
  }

  for (const s of sols) {
    if (!["ABERTA", "AGUARDANDO_APROVACAO"].includes(s.status)) continue;
    const dias = diasDesde(s.atualizado_em ?? s.criado_em);
    if (dias >= 7) g.push({ nivel: "critico", descricao: `Solicitação ${s.numero} sem ação`, detalhe: `${dias} dias em aberto`, href: `/compras/solicitacoes/${s.id}`, linkLabel: "Ver solicitação" });
    else if (dias >= 3) g.push({ nivel: "atencao", descricao: `Solicitação ${s.numero} pendente`, detalhe: `${dias} dias aguardando`, href: `/compras/solicitacoes/${s.id}`, linkLabel: "Ver solicitação" });
  }
  ...
  for (const t of tarefas.slice(0, 3)) {
    ...
    g.push({ nivel: "critico", descricao: `"${t.titulo}" atrasada`, detalhe: `${dias} dia${dias !== 1 ? "s" : ""} em atraso`, href: `/obras/${obraId}?aba=workspace&filtro=atencao`, linkLabel: "Ver workspace" });
  }

  const semDono = tarefas.filter((t) => t.status === "SEM_DONO" && diasDesde(t.criado_em) > 2);
  if (semDono.length > 0)
    g.push({ nivel: "atencao", descricao: `${semDono.length} tarefa${semDono.length > 1 ? "s" : ""} sem responsável`, detalhe: `Aguardando atribuição`, href: `/obras/${obraId}?aba=workspace&filtro=atencao`, linkLabel: "Ver workspace" });
