...
import { NovaCategoriaInline } from "@/modules/squadstock/components/catalogo/nova-categoria-inline";
import { ApagarLinhaBtn } from "@/modules/squadstock/components/catalogo/apagar-linha-btn";
import { ImportarXml } from "@/modules/squadstock/components/catalogo/importar-xml";
import { BtnAlterarUnidade } from "@/modules/squadstock/components/catalogo/btn-alterar-unidade";
...
export default async function LinhaPage({
  ...
  return (
    <div className="px-8 py-8">
      ...
      />
      <BackButton href="/squadstock/catalogo" />
      ...
      <div className="mt-4 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button as="a" href={`/squadstock/catalogo/${params.linhaId}/novo-produto`}>
            Novo produto
          </Button>
        </div>
      </div>
      ...
      {todosTipos.length > 0 && (
        <div className="mt-6 flex overflow-x-auto border-b border-border">
          <Link
            href={`/squadstock/catalogo/${params.linhaId}`}
          ...
          {todosTipos.map((tipo) => (
            <Link
              key={tipo}
              href={`/squadstock/catalogo/${params.linhaId}?tipo=${encodeURIComponent(tipo)}`}
              className={`whitespace-nowrap border-b-2 px-4 py-2 text-sm font-medium -mb-px transition-colors ${
                tipoAtivo === tipo
                  ? "border-primary text-primary"
                  : "border-transparent text-text-2 hover:text-text"
      ...
      <div className="mt-8 space-y-10">
        ...
        {vazio && (
          <div className="card p-10 text-center">
            ...
            {!tipoAtivo && (
              <Button as="a" href={`/squadstock/catalogo/${params.linhaId}/novo-produto`} className="mt-4 inline-flex">
            ...
            return (
              ...
              >
                <td className="px-3 py-2">
                  <Link href={`/squadstock/catalogo/${linhaId}/${p.id}`}>
                    {preview ? (
                      <img
                        src={preview}
                        alt={p.nome}
                ...
                <td className="px-4 py-2.5">
                  <Link
                    href={`/squadstock/catalogo/${linhaId}/${p.id}`}
