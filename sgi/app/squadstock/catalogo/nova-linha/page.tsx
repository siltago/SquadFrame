...
export default async function NovaLinhaPage({
  ...
  // Busca case-insensitive para tolerar variação de caixa na URL. Sem
  // ?tipo= na URL, sempre começa em "Perfil" (aba de longe mais usada) —
  // evita repetir o erro de criar uma linha na aba errada por esquecimento
  // de selecionar o tipo.
  const slugParam = (searchParams.tipo ?? "PERFIL").trim().toUpperCase();
  const tipoPreSelecionado =
    tiposList.find((t) => t.slug.toUpperCase() === slugParam)?.slug ?? tiposList[0]?.slug ?? "";
    ...
    <div className="px-8 py-8">
      ...
      <form action={criarLinha} className="card mt-6 max-w-2xl p-6">
        ...
        <div className="mt-6 flex gap-3">
          ...
          <Button as="a" href={tipoPreSelecionado ? `/squadstock/catalogo?aba=${tipoPreSelecionado}` : "/squadstock/catalogo"} variant="ghost">
            Cancelar
          </Button>
        </div>
      </form>
