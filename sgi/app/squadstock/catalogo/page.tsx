...
import { AbaCoresCatalogo } from "@/modules/squadstock/components/catalogo/aba-cores-catalogo";
import { GerenciarAba } from "@/modules/squadstock/components/catalogo/gerenciar-aba";
import { FilterBar } from "@/modules/squadstock/components/catalogo/filter-bar";
import { GerenciarFornecedores } from "@/modules/squadstock/components/catalogo/gerenciar-fornecedores";
import { GerenciarLinhas } from "@/modules/squadstock/components/catalogo/gerenciar-linhas";
import { GerenciarCategorias } from "@/modules/squadstock/components/catalogo/gerenciar-categorias";
import { Badge } from "@/ui/components/Badge";
import { Button } from "@/ui/components/Button";
import { Pagination } from "@/ui/components/Pagination";
import { EmptyState } from "@/ui/components/EmptyState";
import type { CatalogItem } from "@/modules/squadframe/types/catalogo/catalog-item";
import type { Filters } from "@/modules/squadstock/components/catalogo/filter-bar";
...
function buildFilterUrl(
  ...
  delete next.pagina;
  return `/squadstock/catalogo?${new URLSearchParams(next).toString()}`;
}
    ...
    if (podeBuscar) {
      ...
      items = (produtosRaw ?? []).map((p) => {
        ...
        return {
          ...
          unidade: p.unidade ?? "",
          previewUrl,
          href: `/squadstock/catalogo/${p.linha_id}/${p.id}`,
    ...
    <div className="px-4 py-6 sm:px-8 sm:py-8">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {podeCriar && (
            <Button as="a" href={`/squadstock/catalogo/nova-linha?tipo=${tipoSlug.toLowerCase()}`} variant="secondary" size="sm">
              Nova linha
            </Button>
          )}
          {filtroLinha && (
            <Button as="a" href={`/squadstock/catalogo/${filtroLinha}`} variant="secondary" size="sm">
              Gerenciar linha
            </Button>
          )}
          {podeCriar && filtroLinha && (
            <Button as="a" href={`/squadstock/catalogo/${filtroLinha}/novo-produto`} size="sm">
              Novo item
            </Button>
          )}
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard
          ...
          href={`/squadstock/catalogo?tipo=${tipoSlug.toLowerCase()}`}
          active={!gerenciar}
        />
        <StatCard
          value={totalFornecedores} label="fornecedores"
          href={`/squadstock/catalogo?tipo=${tipoSlug.toLowerCase()}&gerenciar=fornecedores`}
          active={gerenciar === "fornecedores"}
        />
        <StatCard
          value={totalLinhas} label="linhas"
          href={`/squadstock/catalogo?tipo=${tipoSlug.toLowerCase()}&gerenciar=linhas`}
          active={gerenciar === "linhas"}
        />
        <StatCard
          value={totalCategorias} label="categorias"
          href={`/squadstock/catalogo?tipo=${tipoSlug.toLowerCase()}&gerenciar=categorias`}
        ...
        <>
          ...
          {items.length > 0 ? (
            ...
            <div className="mt-3 card flex flex-col items-center justify-center px-6 py-16 text-center">
              ...
              />
              {hasFilters ? (
                <Button as="a" href={`/squadstock/catalogo?tipo=${tipoSlug.toLowerCase()}`} variant="secondary" className="mt-5">
                  Limpar filtros
                </Button>
              ) : podeCriar ? (
                <Button as="a" href={`/squadstock/catalogo/nova-linha?tipo=${tipoSlug.toLowerCase()}`} className="mt-5">
                  Nova linha
                </Button>
              ) : null}
            </div>
