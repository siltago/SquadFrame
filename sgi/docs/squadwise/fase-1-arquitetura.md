...
> **⚠️ SUPERADO em 2026-07-28 — o sistema deixou de ser multi-tenant.** Este documento registra a decisão *original* de fazer o Wise nascer multiempresa (`wise_empresas`, `empresa_id` em toda tabela, RLS escopado por empresa) — decisão revertida depois que ficou claro que o sistema nunca teve, e não vai ter, mais de uma empresa em produção (só existia o seed "sms-esquadrias" e um registro "teste" vazio). Todo o conceito de "empresa"/tenant foi removido do schema e do código (migrations `20260728000001_wise_rls_remove_empresa_scope.sql` e `20260728000002_wise_remove_multiempresa.sql`): a tabela `wise_empresas` e `wise_empresa_modulos` não existem mais, e nenhuma tabela `wise_*` tem `empresa_id`. **Exceção mantida**: `wise_unidades` (filial/local físico) continua existindo — não é multi-tenant, é um conceito diferente (várias unidades da mesma empresa única), só perdeu o `empresa_id` que a escopava por tenant. Onde este documento menciona `empresa_id`/`wise_empresas`/"multiempresa" abaixo, leia como histórico do desenho original, não como o estado atual do sistema — a fonte de verdade atual é o código em `modules/wise/**`.
>
...
>
> **Atualização 2026-07-28**: essa mesma constatação ("nunca existiu mais de uma empresa na prática") foi o motivo pra reverter multiempresa também no Wise — ver aviso no topo do documento.
