...
export async function criarProduto(linhaId: string, formData: FormData) {
  ...
  if (existente) redirect(`/squadframe/catalogo/${existente.linha_id}/${existente.id}`);
...
export async function editarProduto(
  ...
  if (novaLinhaId !== linhaId) {
    revalidatePath(`/squadframe/catalogo/${novaLinhaId}/${produtoId}`);
    return { redirect: `/squadframe/catalogo/${novaLinhaId}/${produtoId}` };
  }
...
export async function deletarProduto(linhaId: string, produtoId: string) {
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/squadframe/catalogo/${linhaId}`);
  redirect(`/squadframe/catalogo/${linhaId}`);
}
