...
export function parseXml(text: string): RascunhoTipologia[] {
  ...
  // ":scope > tag" (só filho direto) — algumas exportações incluem, dentro
  // de cada TIPOLOGIA, os PERFIS que a compõem, cada um com seu próprio
  // QTDE/LARGURA/ALTURA aninhado ANTES dos campos reais da tipologia. Um
  // querySelector(tag) simples pega o primeiro da subárvore inteira (o
  // aninhado, errado); :scope > restringe ao nível da própria tipologia.
  const t = (node: Element, tag: string) =>
    node.querySelector(`:scope > ${tag}`)?.textContent?.trim() ?? "";
  // Algumas exportações usam DESCR, outras DESCRICAO pro mesmo campo (nome
  // descritivo da tipologia) — tenta as duas, na ordem.
  const tAny = (node: Element, tags: string[]) => {
  ...
  return nodes.map((node, idx) => {
    ...
    return {
      ...
      descricao: tAny(node, ["DESCR", "DESCRICAO"]) || null,
