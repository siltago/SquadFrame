...
import { CATALOGO_PERMISSIONS } from "@/modules/squadstock/constants";
...
// Peso do produto-mestre, usado como fallback quando o alias não informa o
// seu próprio peso_metro — mesma herança que o formulário já mostra via
// placeholder, só que resolvida no servidor pra derivar preco_metro.
async function buscarPesoMestre(
  supabase: ReturnType<typeof createClient>,
  produtoId: string
): Promise<number | null> {
  const { data } = await supabase.from("produtos").select("peso_metro").eq("id", produtoId).maybeSingle();
...
export async function criarLinha(formData: FormData) {
  ...
  revalidatePath("/squadstock/catalogo");
  redirect(`/squadstock/catalogo/${data.id}`);
}
...
export async function editarLinha(linhaId: string, formData: FormData) {
  ...
  revalidatePath("/squadstock/catalogo");
  revalidatePath(`/squadstock/catalogo/${linhaId}`);
}
...
export async function apagarLinha(linhaId: string) {
  ...
  revalidatePath("/squadstock/catalogo");
  redirect("/squadstock/catalogo");
}

// ─── Categoria ───────────────────────────────────────────────

export async function criarCategoria(linhaId: string, formData: FormData) {
  await verificarPermissao(CATALOGO_PERMISSIONS.CATEGORIA_GERENCIAR);
  const supabase = createClient();
...
export async function criarProduto(linhaId: string, formData: FormData) {
  ...
  if (existente) redirect(`/squadstock/catalogo/${existente.linha_id}/${existente.id}`);
  ...
  revalidatePath(`/squadstock/catalogo/${linhaId}`);
  redirect(`/squadstock/catalogo/${linhaId}/${data.id}`);
}
...
export async function criarProdutoRapido(dados: {
  ...
  revalidatePath(`/squadstock/catalogo/${dados.linha_id}`);
...
export async function editarProduto(
  ...
  if (novaLinhaId !== linhaId) {
    revalidatePath(`/squadstock/catalogo/${novaLinhaId}/${produtoId}`);
    return { redirect: `/squadstock/catalogo/${novaLinhaId}/${produtoId}` };
  }
...
export async function adicionarAlias(
  ...
  // Barra: o form nunca manda preco_metro direto (é sempre calculado a
  // partir de preco_kg) — deriva e grava aqui pra não depender só do
  // cálculo em tempo de leitura em /api/produtos/search.
  if (row.preco_metro == null && specs?.preco_kg != null) {
    const pesoEfetivo = specs.peso_metro ?? (await buscarPesoMestre(supabase, produtoId));
    if (pesoEfetivo != null) row.preco_metro = Math.round(pesoEfetivo * specs.preco_kg * 100) / 100;
  }
...
export async function editarAlias(
  ...
  // Mesma derivação de adicionarAlias() — barra nunca manda preco_metro
  // direto, sempre calculado a partir de preco_kg.
  if (row.preco_metro == null && specs?.preco_kg != null) {
    const pesoEfetivo = specs.peso_metro ?? (await buscarPesoMestre(supabase, produtoId));
    if (pesoEfetivo != null) row.preco_metro = Math.round(pesoEfetivo * specs.preco_kg * 100) / 100;
  }
...
export async function recalcularPrecoKgPerfisAction() {
  await verificarPermissao(CATALOGO_PERMISSIONS.EDITAR);
  const supabase = createClient();
...
export async function deletarProduto(linhaId: string, produtoId: string) {
  ...
  if (error) throw new Error(error.message);
  revalidatePath(`/squadstock/catalogo/${linhaId}`);
  redirect(`/squadstock/catalogo/${linhaId}`);
}
