...
import { adicionarAlias, editarAlias, excluirAlias } from "@/modules/squadstock/actions/catalogo/actions";
...
function SpecsInputs({ a, tipoUnidade, masterPeso, masterTamanho, masterPrecoKg }: {
  a?: Alias; tipoUnidade?: string | null;
  masterPeso?: number | null; masterTamanho?: number | null; masterPrecoKg?: number | null;
}) {
  const labels = specLabels(tipoUnidade);
  const isBarra = tipoUnidade?.toUpperCase() === "BARRA";
  const showTamanho = !!labels.tamanho;
  const pesoParaCalculo = a?.peso_metro ?? masterPeso ?? null;
  // Alias NOVO (a undefined): pré-preenche com o preço/kg do produto-mestre
  // — sem isso, todo alias novo nascia com "Preço/kg" vazio mesmo quando o
  // produto já tinha uma referência de preço, obrigando redigitar sempre.
  // Alias EXISTENTE com preco_kg nulo: mantém vazio (não "assumido" no
  // useState) — em branco significa "continua herdando dinamicamente do
  // mestre", mesmo comportamento que peso/tamanho já têm via placeholder;
  // pré-preencher aqui faria salvar sem editar "gravar" o valor do mestre
  // no alias, perdendo a herança dinâmica.
  const [precoKg, setPrecoKg] = useState<string>(
    a
      ? (a.preco_kg != null ? String(a.preco_kg) : "")
      : (masterPrecoKg != null ? String(masterPrecoKg) : "")
  );
        ...
        {isBarra ? (
          <div className="min-w-[80px] flex-1">
            ...
            <input name="preco_kg" type="number" step="any" min="0"
              ...
              className="field h-7 text-xs"
              placeholder={masterPrecoKg ? `${masterPrecoKg} (mestre)` : "Ex: 18.50"} />
          </div>
        ) : (
          <div className="min-w-[80px] flex-1">
...
function AliasRow({ a, produtoId, linhaId, tipoUnidade, fornecedoresDisponiveis, coresVinculadas, masterPeso, masterTamanho, masterPrecoKg }: {
      ...
      <tr className="border-b border-border last:border-0 bg-bg">
        <td colSpan={5} className="px-4 py-3">
          <form onSubmit={salvar} className="space-y-2">
            ...
            <SpecsInputs a={a} tipoUnidade={tipoUnidade} masterPeso={masterPeso} masterTamanho={masterTamanho} masterPrecoKg={masterPrecoKg} />
...
export function AbaAliases({ produtoId, linhaId, aliases, tipoUnidade, fornecedoresDisponiveis = [], coresVinculadas = [], masterFornecedorId, masterPeso, masterTamanho, masterPrecoKg }: {
    ...
    <div className="mt-6 space-y-4">
            ...
            <tbody>
              {aliases.map((a) => (
                <AliasRow key={a.id} a={a} produtoId={produtoId} linhaId={linhaId}
                  ...
                  masterPeso={masterPeso} masterTamanho={masterTamanho} masterPrecoKg={masterPrecoKg} />
              ))}
            </tbody>
          </table>
        </div>
      ...
      {showForm ? (
        <form onSubmit={handleSubmit} className="card p-4 space-y-3">
          ...
          <SpecsInputs tipoUnidade={tipoUnidade} masterPeso={masterPeso} masterTamanho={masterTamanho} masterPrecoKg={masterPrecoKg} />
          {erro && <p className="text-xs text-danger">{erro}</p>}
